<script>
  // importar componente para exibir itens da grid (tarefas)
  ...
  function getTasks() {
    fetch("https://svelte-5d5e4.firebaseio.com/tasks.json")
      .then(response => {
        if (!response.ok) {
  ...
  function updateTask() {
    ...
    fetch(`https://svelte-5d5e4.firebaseio.com/tasks/${id}.json`, {
      method: "PATCH",
      body: JSON.stringify(taskData),
      headers: { "Content-Type": "application/json" }
    })
...
<div class="container">
  <div class="notification">
    ...
    <!-- Binding -->
    <div class="field is-horizontal {id ? 'updating' : 'adding'}-record">
      <div class="field-label">{id ? 'Editing' : 'New'} record:</div>
      <div class="field-body">
        <div class="field">
